(function(){
  ...
  function drawFW(){
    const fx=W*.22,fy=H*.38,R=FW.R;
    cx.save();cx.globalAlpha=.7;
    cx.beginPath();cx.arc(fx,fy,R,0,Math.PI*2);cx.strokeStyle='rgba(0,229,255,.12)';cx.lineWidth=1.5;cx.stroke();
    cx.beginPath();cx.arc(fx,fy,R*.15,0,Math.PI*2);cx.strokeStyle='rgba(0,229,255,.2)';cx.lineWidth=1;cx.stroke();
    for(let i=0;i<8;i++){
      const a=FW.rot+(i/8)*Math.PI*2,ex=fx+Math.cos(a)*R,ey=fy+Math.sin(a)*R;
      cx.beginPath();cx.moveTo(fx,fy);cx.lineTo(ex,ey);cx.strokeStyle='rgba(0,229,255,.08)';cx.lineWidth=1;cx.stroke();
  ...
  function drawRC(){
    ...
    cx.beginPath();pts.forEach(([x,y],i)=>i===0?cx.moveTo(x,y):cx.lineTo(x,y));
    cx.strokeStyle='rgba(0,229,255,.15)';cx.lineWidth=1.5;cx.stroke();cx.restore();
  }
  let frame=0;
  function draw(){
    ...
    const glow=cx.createRadialGradient(W*.8,H*.5,0,W*.8,H*.5,W*.6);
    glow.addColorStop(0,'rgba(0,229,255,.04)');glow.addColorStop(1,'transparent');
    cx.fillStyle=glow;cx.fillRect(0,0,W,H);
    cx.strokeStyle='rgba(0,229,255,.03)';cx.lineWidth=1;
